...
Spark.prototype = {
  ...
  init: function () {
    ...
    this.angle = (Math.random() * 360 * Math.PI) / 180; //* 60 + 60
    ...
    this.multiplier = 500; //* Math.random() + 4000;
    ...
    this.x = 0; //0.2 - 0.4 * Math.random();
    this.y = 0; //0.2 * Math.random();
  },
  ...
  move: function (time) {
    ...
    // if (Math.abs(this.x) >= 1) this.dx *= -1;

    // Обновляем координаты искры
    this.x +=
      this.dx * (Math.pow(this.radius, 2) - Math.pow(this.y, 2) * speed);
    this.y +=
      this.dy * (Math.pow(this.radius, 2) - Math.pow(this.x, 2) * speed); //* this.x * this.x

    // Если искра достигла конечной точки, перезапускаем ее
    if (Math.abs(this.y) > 3 || Math.abs(this.x) > 3) {
      //this.init();
    }
  },
};
...
// Инициализация WebGL
var canvas = document.getElementById("glCanvasEleven");
