...
Spark.prototype = {
  ...
  init: function () {
    ...
    this.angle = ((Math.random() * 100 + 40) * Math.PI) / 180;

    // Радиус - это расстояние, которое пролетит искра
    this.radius = 3;

    // Вычисляем максимальные координаты искры на окружности
    this.xMax = Math.cos(this.angle);
    this.yMax = -Math.sin(this.angle);
    ...
    this.x = -0.5 + 0.8 * Math.random();
    this.y = 3.7 * Math.random();
  },
  ...
  move: function (time) {
    ...
    if (Math.abs(this.y) > 3 || Math.abs(this.x) > 2.9) {
      this.init();
    }
  },
};
...
// Инициализация WebGL
var canvas = document.getElementById("glCanvasTen");
